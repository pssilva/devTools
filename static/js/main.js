...
$.mainBuscaFacil = { 
    ...
    init: function(){
      ...
      // $flightDepartureDate.attr("placeholder",convertDate());
      // $flightArrivalDate.attr("placeholder",convertDate());
    },
    autocomplete: function(){ 
        var cache = {};

        $(document).on('focus', '.select-flight', function(){
            $(this).autocomplete({ 
                source: function( request, response ) { 
                    ...
                    $.getJSON("static/data/airports_BRA.json", request, function(data,status,xhr) {
                        var result=[]; 
                ...
                select: function( event, ui ) { 
                    $((this.id == 'select-flight') ? '#departure' : '#arrival').val(ui.item.code);
                ...
                change: function( event, ui ) {
                    ...
                    if(ui.item == null){
                        $((this.id == 'select-flight') ? '#departure' : '#arrival').val('');
                        this.value = ''; 
                    }
                }
    ...
    searchCallback: function(template,data){
        ...
        $("#list-flights").html(html).show(400, function(){
        ...
        $(".cf").on('click', function(event){
            ...
            var selectFlightCallback = function() {
                ...
                if(paymentObj.INV.PRICE_SUM > 0){
                    ...
                    var cia = ($(this).parent().find('.sortCia').text()).toLowerCase();
            ...
            var clickFlightCallback = function() {
               //////////////////////////////////////////////////////////
                ...
                if(!$('#dados-passageiros').hasClass('done')){
    ...
    filtroFix: function(){   
      ...
      // if($summaryPurchase.length > 0){
      //   var $listOutbound = $('#list-outbound'), 
      //     $filterOptions = $(".filter-options-outbound"), 
      //     filterOptionsTop = $filterOptions.offset().top, 
      //     summaryPurchaseTop = $summaryPurchase.offset().top, 
      //     summaryPurchaseHeight = $summaryPurchase.height() + parseInt($summaryPurchase.css('padding-top').replace('px','')) + 2, 
      //     summaryPurchaseEnd = summaryPurchaseTop+summaryPurchaseHeight;
      // }
      
      // var $listInbound = $('#list-outbound'), 
      //     $filterOptionsIn = $(".filter-options-inbound");

      // if($filterOptionsIn.length > 0){
      //   var listInboundTop = $listInbound.offset().top, 
      //       listInboundHeight = $listInbound.height(), 
      //       filterOptionsInTop = $filterOptionsIn.offset().top, 
      //       filterOptionsInHeight = $filterOptionsIn.height(), 
      //       filterOptionsInEnd = filterOptionsInTop+filterOptionsInHeight, 
      //       heightIn = listInboundHeight + summaryPurchaseHeight; 
      //   }

      // $(window).scroll(function (eventData){ 
      //   if($summaryPurchase.length > 0){
      //     listInboundTop = $('#list-inbound').offset().top;
          
      //     var ultimoFilho = $('#list-outbound > div > div:last-child');
      //     var ultimoFilhoTop =  ultimoFilho.offset().top;
      //     var ultimoFilhoHeight =  ultimoFilho.height() + ultimoFilhoTop;
      ...
      ////////////////////
        // outra versao funcionando com somente ida
        ////////////////////////////////////////////
    ...
    clickManuSort: function(){
      ...
      $('.menu-filter li a').on('click', function (event) {
        var $parentDiv = $(this).parents( ".filter-options-outbound");
        if (!$parentDiv.length){
          $parentDiv = $(this).parents( ".filter-options-inbound");
        }

      var keyObj = mainBuscaFacil.object_keys($(this).data());

      console.log("$(this).data()");
      console.log($(this).data());

      if($parentDiv.hasClass("filter-options-inbound")){
        mainBuscaFacil.inBoundList.filter(); 
      }else{
        mainBuscaFacil.outBoundList.filter();
      }
      ...
      switch(keyObj[0]) {         
        case 'sortLegs': 
          filterBy = $(this).data()[keyObj[1]];
          if(mainBuscaFacil.outBoundList != null){

            var vOrder = ($(this).data()[keyObj[0]]=='vd')?"asc":"desc";
            
            if($parentDiv.hasClass("filter-options-inbound")){
              mainBuscaFacil.inBoundList.sort(filterBy, { order: vOrder });
              $('#checkout-return > div.cia-not-defined  > span').trigger('click');
            }else{
              mainBuscaFacil.outBoundList.sort(filterBy, { order: vOrder });
        ...
        case 'duration':          
          ...
          if(mainBuscaFacil.outBoundList != null){
            ...
            var vOrder = ($(this).data()[keyObj[0]]=='vmc')?"asc":"desc";
        ...
        case 'departureTime': 
          ...
          if(mainBuscaFacil.outBoundList != null){
            ...
            var vOrder = ($(this).data()[keyObj[0]]=='vmcd')?"asc":"desc";
        ...
        break; 
        case 'priceMilhasfacil':          
          filterBy = $(this).data()[keyObj[1]];
          if(mainBuscaFacil.outBoundList != null){

            var vOrder = ($(this).data()[keyObj[0]]=='mb')?"asc":"desc";
        ...
        case 'earnMoney':          
          ...
          if(mainBuscaFacil.outBoundList != null){
            var vOrder = ($(this).data()[keyObj[0]]=='me')?"desc":"asc";
        ...
        case 'hasPromo':          
          ...
          if(mainBuscaFacil.outBoundList != null){
            ...
            var vOrder = ($(this).data()[keyObj[0]]=='pp')?"desc":"asc";
        ...
        case 'sortCia':          
          ...
          if(mainBuscaFacil.outBoundList != null){            
            ...
            if($parentDiv.hasClass("filter-options-inbound")){        
              ...
              mainBuscaFacil.inBoundList.filter(function(item) { 
                  return (item.values().sortCia == filterBy);
              });               
              $('#checkout-return > div.cia-not-defined  > span').trigger('click');
            }else{
    ...
    steps: function(){
       /////////// Step ///////////
      var mainBuscaFacil = this;
      var next_fs = $('.step2').addClass("hide");      
